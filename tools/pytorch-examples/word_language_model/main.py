...
parser.add_argument('--logfile', type=str, default='',
                    help='path to save per-epoch perplexity log as CSV '
                         '(columns: epoch, train_ppl, valid_ppl; '
                         'test_ppl is appended as a final row)')
...
def train():
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        ...
        if args.dry_run:
            break
    return epoch_loss / max(1, epoch_batches)
...
if args.logfile:
    log_file_handle = open(args.logfile, 'w', newline='')
    log_writer = csv.writer(log_file_handle)
    log_writer.writerow(['epoch', 'train_ppl', 'valid_ppl'])
...
try:
    for epoch in range(1, args.epochs+1):
        ...
        print('-' * 89)
        if log_writer:
            log_writer.writerow([epoch, f'{math.exp(train_loss):.4f}', f'{math.exp(val_loss):.4f}'])
...
if log_writer:
    log_writer.writerow(['test', '', f'{math.exp(test_loss):.4f}'])
